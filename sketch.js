...
function setup() {
  ...
  missile = createSprite(400,500);
     ...
     enemiesGroup.debug=true;
      newSoldierimg.debug=true;
      newSoldierimg.setCollider("rectangle",10,40,90,390);
}
  ...
  if (gameState === PLAY) {
    ...
    if (frameCount >= 490) {
     ...
     //Soundloop=false;
      ...
      //Znew=createSprite(490,80);
      //Znew.addImage(Zlimg);
      //Znew.scale=0.4;

      //Snew=createSprite(490,80);
      //Snew.addImage(Shootimg);
      //Snew.scale=0.4;
      ...
      if (enemiesGroup.isTouching(newSoldierimg) || EbulletGroup.isTouching(newSoldierimg)) {
